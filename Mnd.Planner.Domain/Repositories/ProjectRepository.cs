...
namespace Mnd.Planner.Domain.Repositories
{
    public class ProjectRepository
    {
        ...
        public Project GetProjectWithWorkload(int projectId, int releaseId, int milestoneId, int deliverableId)
        {
            var project = this.GetProject(projectId);
            
            var _assignments = new List<ResourceAssignment>();
            ...
            using (var conn = new SqlConnection(this.ConnectionString))
            {
                ...
                using (var resourcesReader = cmdResources.ExecuteReader())
                {
                    while (resourcesReader.Read())
                    {
                        ...
                        var resource = new Resource { Id = int.Parse(resourcesReader["PersonId"].ToString()), FirstName = resourcesReader["FirstName"].ToString(), MiddleName = resourcesReader["MiddleName"].ToString(), LastName = resourcesReader["LastName"].ToString() };
                        ...
                        _assignments.Add(new ResourceAssignment
                        {
                            ...
                            Phase = new Phase { Id = int.Parse(resourcesReader["PhaseId"].ToString()), Title = resourcesReader["phasetitle"].ToString() },
                            Resource = resource,
                            Project = new Project { Id = int.Parse(resourcesReader["ProjectId"].ToString()), Title = resourcesReader["ProjectTitle"].ToString() },
                            //TODO: fill ActivitiesNeeded
                            Deliverable = new Deliverable { Id = int.Parse(resourcesReader["DeliverableId"].ToString()), Title = resourcesReader["DeliverableTitle"].ToString() },
                            Milestone = new Milestone { Id = int.Parse(resourcesReader["MilestoneId"].ToString()), Title = resourcesReader["MilestoneTitle"].ToString() },
                            StartDate = DateTime.Parse(resourcesReader["StartDate"].ToString()),
                            EndDate = DateTime.Parse(resourcesReader["EndDate"].ToString()),
                            Activity = new Activity { Id = int.Parse(resourcesReader["ActivityId"].ToString()), Title = resourcesReader["ActivityTitle"].ToString() }
                        });
                    }
                }
            }
                        ...
                        var status = new ActivityStatus
                                        {
                                            ...
                                            Activity = new Activity { Id = int.Parse(statusReader["ActivityId"].ToString()), Title = statusReader["ActivityTitle"].ToString(), Description = statusReader["ActivityDescription"].ToString() },
                                        };
